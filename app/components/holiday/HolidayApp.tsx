...
export default function HolidayApp() {
  ...
  // Brief "searching" flash whenever inputs change — feels alive.
  useEffect(() => {
    setLoading(true)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-6 pt-20 pb-28 md:pt-28 md:pb-40">
          ...
          <div className="mt-6 flex items-center gap-2 flex-wrap justify-center">
            <span className="text-white/70 text-xs font-semibold uppercase tracking-wider mr-1">
              Popular:
            </span>
            {POPULAR.map((d) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setParams((prev) => ({ ...prev, destination: `${d.name}, ${d.tag}` }))
                }
        ...
        {/* wavy divider */}
        <svg
          className="relative block w-full -mb-px"
          viewBox="0 0 1440 80"
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-14">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {cheapestStay && showStays && (
              <StatCard
                icon="payments"
                label="Cheapest stay"
                value={`${cheapestStay.currency}${cheapestStay.pricePerNight}/nt`}
                sub={cheapestStay.providerName}
                color="#10b981"
              />
            )}
            {cheapestFlight && showFlights && (
              <StatCard
                ...
                value={`${cheapestFlight.currency}${cheapestFlight.price}`}
                sub={cheapestFlight.airline}
                color="#6366f1"
              />
            )}
          </div>
            ...
            <div className="flex items-center gap-2 text-sm">
              {loading ? (
                ...
                <span className="text-slate-600">
                  ...
                  {showFlights && (
                    <>
                      {' '}· <span className="font-bold text-slate-900">{flights.length}</span> flights
                    </>
                  )}
                </span>
              )}
            ...
            <div className="inline-flex rounded-xl bg-white border border-slate-200 p-1 text-sm">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg font-semibold transition-colors ${
                  sortBy === 'price' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Cheapest first
              </button>
        ...
        {showStays && (
          <div className="mb-12">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {sortedStays.map((s, i) => (
                  <StayCard key={s.id} deal={s} rank={i} />
                ))}
              </div>
            )}
          </div>
        ...
        {/* Provider strips — the real deep-link aggregator */}
        <div className="grid gap-5 lg:grid-cols-2 mt-8">
          {showStays && (
            <ProviderStrip
              title="Also searched for stays on"
              providers={STAY_PROVIDERS}
              params={params}
            />
          )}
          {showFlights && (
            <ProviderStrip
              title="Also searched for flights on"
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 pb-16">
        <div className="grid md:grid-cols-3 gap-4">
          <TrustCard
            icon="verified"
            title="Every major site, one page"
            body="We search 21+ providers — Booking, Airbnb, Vrbo, Expedia, Skyscanner, Kayak, Agoda and more — so you don't have to."
          />
          <TrustCard
            icon="price_check"
            title="Always the lowest price"
            body="Sorted cheapest-first by default. Savings tags show you how much you're knocking off the standard rate."
          />
          <TrustCard
            icon="shield_person"
            title="You book direct"
            body="No hidden fees. Click through and book directly with the provider of your choice — at their live price."
          />
        </div>
      </section>
    ...
    <header className="absolute top-0 left-0 right-0 z-30">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="bg-white/15 hover:bg-white/25 text-white text-sm font-semibold rounded-full px-4 py-2 border border-white/25 backdrop-blur-md transition-colors">
            Sign in
          </button>
        </div>
      </div>
